...
interface BagProps {
  item: CartItemT;
  onUpdateCart: () => void; // Callback to trigger parent total calculation
}

const BagItem: FC<BagProps> = ({ item, onUpdateCart }) => {
  // Encapsulate quantity state locally, derived from the database single source of truth
  ...
  const updateCart = (id: string, action: string) => {
    ...
    axios.put("cart/update/", { productId: id, action }, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then(() => {
        setQuantity(newQuantity);
        showToast("success", "Cart updated");
        onUpdateCart(); // Syncs the new total pricing with the parent component
      })
      .catch((error) => {
        if (error.response?.data?.message === "Unauthorized access") {
          Swal.fire({
  ...
  return (
    <div className="flex items-center justify-between border-b py-4">
      <div className="flex items-center">
        ...
        <div className="flex flex-col justify-between py-1">
          ...
          <div className="flex items-center rounded-full">
            <button
              onClick={() => updateCart(item.productId?._id, "minus")}
              className="py-1 text-gray-600 focus:outline-none disabled:opacity-50"
              disabled={quantity <= 1 || !item.productId?._id}
            >
              <span className="px-2 py-[1px] border bg-[#EAF2FF] text-[#006838] rounded-full text-sm font-bold">
                -
              </span>
      ...
      <div className="flex items-center space-x-4">
        <span className="font-extrabold text-gray-900">
          ₦ {(safePrice * quantity).toLocaleString()}
        </span>
      </div>
    </div>
  );

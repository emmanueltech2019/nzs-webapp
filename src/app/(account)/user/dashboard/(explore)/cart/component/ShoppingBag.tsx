...
const ShoppingBag = () => {
  ...
  useEffect(() => {
    ...
    // If there is no token, stop loading
    if (!token) {
      setLoading(false);
      return;
    }
    ...
    const fetchCartAndProfile = async () => {
      try {
        ...
        // Fetch cart and profile at the same time
        ...
        // Log the complete responses for debugging
        console.log("Cart response:", cartResponse.data);
        console.log("Profile response:", profileResponse.data);

        // Safely get cart
        const cart = cartResponse.data?.cart ?? null;

        // Safely get total
        const cartTotal = Number(cartResponse.data?.total ?? 0);

        // Safely get wallet balance
        const walletBalance = Number(
          profileResponse.data?.wallet?.balance ?? 0
        );
        ...
        setCartItems(cart);
        setTotal(Number.isFinite(cartTotal) ? cartTotal : 0);
        setBalance(Number.isFinite(walletBalance) ? walletBalance : 0);
      } catch (error: any) {
        console.error("Cart/Profile error:", error);
        ...
        // Log other errors without crashing the application
        console.error("Unable to load cart or profile:", {
  ...
  if (loading) {
    return (
      <div className="w-screen md:mx-1 bg-white md:p-4 pr-6 pl-2 md:w-[53vw] min-h-screen">
        <TagHeader
          title="Basket"
          rightData={`₦ ${balance.toLocaleString("en-NG")}`}
        />
  ...
  // Safely get items
  const items: CartItemT[] = Array.isArray(cartItems?.items)
    ? cartItems.items
    : [];
  ...
  return (
    <div className="w-screen md:mx-1 bg-white md:p-4 pr-6 pl-2 md:w-[53vw]">
      ...
      <div className="overflow-y-auto h-[60vh]">
        {items.length > 0 ? (
          items.map((item: CartItemT, index: number) => {
            // Log every cart item for debugging

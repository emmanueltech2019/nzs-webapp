...
const AddLegalServices = (props: Props) => {
    ...
    const {setAddLegalServices, LegalSpecialtyTitle, setLegalSpecialtyTitle, LegalBrieflyDescribe, setLegalBrieflyDescribe, legalSimilarSpecialties, setLegalSimilarSpecialties, legalServicesAvailable, setLegalServicesAvailable, legalServiceCost, setLegalServiceCost, legalProviderType, setLegalProviderType, legalProviderName, setLegalProviderName, legalProviderProfileUrl, setLegalProviderProfileUrl, legalBookingInfo, setLegalBookingInfo, setLegalBookingTitle, legalBookingTitle, handleSaveLegalData} = useProvider();
    const [legalSpecialServicesCollection, setLegalSpecialServicesCollection] = useState<string>("")
    const [activeTime, setActiveTime] = useState<string>("")
    const [activeProcedure, setActiveProcedure] = useState<string>("");
    const legalSimilarSpecialtyArray = ["CRIMINAL LAW", "PROPERTY LAW", "FAMILY LAW", "CORPERATE", "TAXATION", "EMPLOYMENT"];
    const legalServicesAvailableArray = ["CONSULTATION", "REPRESENTATION", "ADVICE", "DOCUMENTATION", "CALL", "RETAIN"];
    const legalProviderTypeArray = ["PARTNER", "ASSOCIATE", "PARALEGAL", "LEGAL ASSISTANT", "CONSULTANT"];
  ...
  return (
    <div className='flex flex-col gap-10 w-full'>
        <div className="flex justify-between items-center">
            <ChevronLeft onClick={() => setAddLegalServices(false)} size={30} strokeWidth={1} className='text-gray-300' />
        ...
        {currentIndex === 0 ? (
            <div className="flex flex-col gap-5">
                <div className="flex flex-col gap-2">
                    <input onChange={(e) => setLegalSpecialtyTitle(e.target.value)} type='text' placeholder='Course Title' className='w-full border-gray-100 border p-3 rounded-xl outline-none' />
                    <input onChange={(e) => setLegalBrieflyDescribe(e.target.value)} type='text' placeholder='Briefly Describe' className='w-full border-gray-100 border p-3 rounded-xl outline-none' />
                    <p className="text-[10px] text-gray-300">0/30</p>
                </div>
                <TextNum text='Similar Specialties' optionalText='List of Available Categories' number={9} />
                <div className="flex flex-wrap gap-3">
                    {legalSimilarSpecialtyArray.map((sec, index) => (
                        <CustomButton handleClick={() => {setLegalSimilarSpecialties(prev => [...prev, sec]), setLegalSpecialServicesCollection(sec)}} setActive={setLegalSpecialServicesCollection} active={legalSpecialServicesCollection} key={index} text={sec} />
                    ))}
                </div>
                <TextNum text='Student Category' optionalText='Select a category' number={9} />
                <div className="flex flex-wrap gap-3">
                    {legalServicesAvailableArray.map((sec, index) => (
                        <CustomButton handleClick={() => {setLegalServicesAvailable(prev => [...prev, sec]), setLegalSpecialServicesCollection(sec)}} setActive={setLegalSpecialServicesCollection} active={legalSpecialServicesCollection} key={index} text={sec} />
                    ))}
                </div>
                <div className="flex flex-col gap-2 w-full">
                    <div className="border border-gray-100 rounded-xl flex flex-col gap-5 p-3">
                        ...
                        <input onChange={(e) => setLegalServiceCost(e.target.value)} type='number' placeholder='Maximum Capacity' className='w-full border-gray-100 border p-3 rounded-xl outline-none' />
                    </div>
                </div>
            </div>   
        ) : currentIndex === 1 ? (
            <div className="flex flex-col gap-5">
                ...
                {activeProvider === 1 ? (
                    <>
                        ...
                        <div className="flex flex-wrap gap-3">
                            {legalProviderTypeArray.map((provider, index) => (
                                <CustomButton handleClick={() => setLegalProviderType(prev => prev.includes(provider) ? prev.filter(item => item !== provider) : [...prev, provider])} key={index} text={provider} />
                            ))}
                        </div>
                        <div className="flex flex-col gap-3">
                            <input onChange={(e) => setLegalProviderName(prev => {
                                const newNames = [...prev];
                            ...
                            <input onChange={(e) => setLegalProviderProfileUrl(prev => {
                                const newNames = [...prev];
                                newNames[activeProvider] = e.target.value;
                                return newNames
                    ...
                    <>
                        <div className="flex justify-between items-center w-full">
                            <div className="flex flex-col relative w-15 h-8">
                                <div onClick={() => setActiveTime(prev => prev === "year" ? "" : "year")} className="w-full h-full flex items-center gap-2 border border-gray-100 rounded-lg p-1 text-gray-400 cursor-pointer">
                                    <p className="">{legalBookingInfo.year || "YEAR"}</p>
                                ...
                                <div ref={hourRef} className="flex flex-col gap-3 absolute top-10 left-0 w-full bg-white shadow" style={{
                                    ...
                                    {bookingYear.map((year, index) => (
                                        <p onClick={() => setLegalBookingInfo(prev => ({...prev, year: year.toString()}))} key={index} className="text-gray-400 w-full text-center bg-gray-100 rounded-lg">{year}</p>
                                    ))}
                                </div>
                            </div>
                            <div className="flex flex-col relative w-15 h-8">
                                <div onClick={() => setActiveTime(prev => prev === "month" ? "" : "month")} className="w-full h-full flex items-center gap-2 border border-gray-100 rounded-lg p-1 text-gray-400 cursor-pointer">
                                    <p className="">{legalBookingInfo.month || "MONTH"}</p>
                                    <ChevronDown strokeWidth={1} size={20} />
                                </div>
                                <div ref={hourRef} className="flex flex-col gap-3 absolute top-10 left-0 w-full bg-white shadow" style={{
                                    ...
                                    {bookingMonth.map((month, index) => (
                                        <p onClick={() => setLegalBookingInfo(prev => ({...prev, month: month}))} key={index} className="text-gray-400 w-full text-center bg-gray-100 rounded-lg">{month}</p>
                                    ))}
                                </div>
                            </div>
                        </div>
                        <div className="flex justify-between items-center gap-3">
                            {bookingDay.map((day, index) => (
                                <p onClick={() => setLegalBookingInfo(prev => ({...prev, day: day}))} key={index} className={`${(day === "Sa" || day === "Su") && "text-red-500"} ${legalBookingInfo.day === day && "bg-[#eaf2ff]"} md:w-12 md:h-12 w-8 h-8 flex justify-center items-center rounded-xl border hover:scale-75 transition-transform duration-50 active:scale-50 cursor-pointer select-none hover:bg-[#eaf2ff]`}>{day}</p>
                            ) )}
                        </div>
                        <div className="flex flex-wrap gap-3 justify-stretch">
                            {bookingDate.map((date, index) => (
                                <p onClick={() => setLegalBookingInfo(prev => ({...prev, date: date.toString()}))} key={index} className={`md:w-12 md:h-12 w-8 h-8 flex justify-center items-center rounded-xl border hover:scale-75 transition-transform duration-50 active:scale-50 cursor-pointer select-none hover:bg-[#eaf2ff] ${legalBookingInfo.date === date.toString() && "bg-[#eaf2ff]"}`}>{date}</p>
                            ) )}
                        </div>
                        <input onChange={(e) => setLegalBookingTitle(prev => {
                                const newData = [...prev];
                            ...
                            <div className="flex gap-3 items-center">
                                <div className="flex flex-col relative w-15 h-8">
                                    <div onClick={() => setActiveTime(prev => prev === "hour" ? "" : "hour")} className="w-full h-full flex items-center gap-2 border border-gray-100 rounded-lg p-1 text-gray-400">
                                        <p className="">{legalBookingInfo.hour.padStart(2, "0") || "00"}</p>
                                        <ChevronDown strokeWidth={1} size={20} />
                                    </div>
                                    <div ref={hourRef} className="flex flex-col gap-3 absolute top-10 left-0 w-full" style={{
                                        ...
                                        {bookingHour.map((hour, index) => (
                                            <p onClick={() => setLegalBookingInfo(prev => ({...prev, hour: hour.toString()}))} key={index} className="text-gray-400 w-full text-center bg-gray-100 rounded-lg">{hour}</p>
                                        ))}
                                    </div>
                                </div>
                                <div className="flex flex-col relative w-15 h-8">
                                    <div onClick={() => setActiveTime(prev => prev === "min" ? "" : "min")} className="w-full h-full flex items-center gap-2 border border-gray-100 rounded-lg p-1 text-gray-400">
                                        <p className="">{legalBookingInfo.min.padStart(2, "0") || "00"}</p>
                                        <ChevronDown strokeWidth={1} size={20} />
                                    </div>
                                    <div ref={minRef} className="flex flex-col gap-3 absolute top-10 left-0 w-full overflow-hidden" style={{
                                        ...
                                        {bookingMin.map((min, index) => (
                                            <p onClick={() => setLegalBookingInfo(prev => ({...prev, min: min.toString()}))} key={index} className="text-gray-400 w-full text-center bg-gray-100 rounded-lg">{min}</p>
                                        ))}
                                    </div>
                                </div>
                                <div className="flex flex-col relative w-15 h-8">
                                    <div onClick={() => {setActiveTime(prev => prev === "meridiem" ? "" : "meridiem")}} className="w-full h-full flex items-center gap-2 border border-gray-100 rounded-lg p-1 text-gray-400">
                                        <p className="">{legalBookingInfo.meridiem || "AM"}</p>
                                        <ChevronDown strokeWidth={1} size={20} />
                                    </div>
                                    <div ref={meridiemRef} className="flex flex-col gap-3 absolute top-10 left-0 w-full overflow-hidden" style={{
                                        ...
                                        {meridiem.map((mer, index) => (
                                            <p onClick={() => setLegalBookingInfo(prev => ({...prev, meridiem: mer}))} key={index} className="text-gray-400 w-full text-center bg-gray-100 rounded-lg">{mer}</p>
                                        ))}
                                    </div>
                                </div>
                            </div>
            ...
            <div onClick={() => setAddHealthPreviewDrop(!addHealthPreviewDrop)} className="bg-gray-100 p-3 rounded-xl flex flex-col gap-3">
                <div className={`flex justify-between items-center gap-5`}>
                    ...
                    <p className="md:text-md text-sm">{LegalSpecialtyTitle}</p>
                    </div>
                    <div className="flex items-center gap-2">
                ...
                <div className={`bg-white transition-all duration-200 rounded-xl flex flex-col gap-3 overflow-hidden mt-3`}>
                    <p className="text-sm md:text-md text-[#c3cad9] p-3">{LegalBrieflyDescribe}</p>
                    <div className="flex flex-col gap-5 p-3">
                        {legalProviderName.length > 0 && (
                            <>
                                {
                                    legalProviderName.map((name, index) => (
        ...
        <div className="flex flex-col gap-3 w-1/2 fixed bottom-0  left-1/2 -translate-x-1/2 p-5 h-50">
            {currentIndex === addHealthStepArray.length - 1 ? (
                <button onClick={() => {handleSaveLegalData(), setAddLegalServices(false)}} className='bg-[#006838] text-white rounded-2xl p-5 flex-grow'>CONFIRM AND SAVE</button>
            ) : (
                <>
                     <div onClick={() => setAddHealthPreviewDrop(!addHealthPreviewDrop)} className="bg-gray-100 p-3 rounded-xl flex flex-col gap-3">
                        <div className="flex justify-between items-center">

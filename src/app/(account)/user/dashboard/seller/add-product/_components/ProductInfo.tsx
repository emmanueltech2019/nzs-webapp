...
const BusinessDescription: FC<general_type> = ({ handleBtnFunc, setCount, setSection }) => {
  ...
  const handleSelection = (
    ...
    setState: React.Dispatch<React.SetStateAction<{ item: string; state: boolean }[]>>,
    selectedItem: string
  ) => {
    setState((prev) =>
      prev.map(({ item, state }) => ({
  ...
  return (
    <div className='py-3 pb-5'>
      ...
      <Accordion title='Product Type' subTitle='Select up to 2 options' onClick={toggleProductType} state={isProductTypeOpen}>
        <div className='flex gap-2 flex-wrap'>
      ...
      <Accordion title='Color' subTitle='Select up to' onClick={toggleColor} state={isColorOpen}>
        <div className='flex gap-3 flex-wrap'>
          {colorState.map(({ item, state }) => (
            <button
              key={item}

...
const cities = [
  { id: 0, abbr: "", name: "Select city" },
  { id: 4, abbr: "ABA", name: "ABA" },
  { id: 8, abbr: "AKK", name: "ABAKALIKI" },
  { id: 5, abbr: "ABK", name: "ABEOKUTA" },
  { id: 6, abbr: "ABV", name: "ABUJA" },
  { id: 7, abbr: "ADK", name: "ADO EKITI" },
  { id: 9, abbr: "AKR", name: "AKURE" },
  { id: 10, abbr: "ASB", name: "ASABA" },
  { id: 11, abbr: "AWK", name: "AWKA" },
  { id: 12, abbr: "BAU", name: "BAUCHI" },
  { id: 14, abbr: "BNI", name: "BENIN" },
  { id: 13, abbr: "BRK", name: "BIRNIN- KEBBI" },
  { id: 15, abbr: "BNY", name: "BONNY ISLAND" },
  { id: 16, abbr: "CBQ", name: "CALABAR" },
  { id: 17, abbr: "DAM", name: "DAMATURU" },
  { id: 18, abbr: "DUT", name: "DUTSE" },
  { id: 19, abbr: "EKT", name: "EKET" },
  { id: 20, abbr: "ENU", name: "ENUGU" },
  { id: 21, abbr: "GOM", name: "GOMBE" },
  { id: 22, abbr: "GUS", name: "GUSAU" },
  { id: 3, abbr: "IBA", name: "IBADAN" },
  { id: 24, abbr: "IJB", name: "IJEBU" },
  { id: 25, abbr: "IKP", name: "IKOT EKPENE" },
  { id: 23, abbr: "IFE", name: "ILE-IFE" },
  { id: 65, abbr: "Ile", name: "Ilesha" },
  { id: 26, abbr: "ILR", name: "ILORIN" },
  { id: 27, abbr: "JAL", name: "JALINGO" },
  { id: 28, abbr: "JOS", name: "JOS" },
  { id: 29, abbr: "KAD", name: "KADUNA" },
  { id: 30, abbr: "KAN", name: "KANO" },
  { id: 31, abbr: "KAS", name: "KASTINA" },
  { id: 32, abbr: "LAF", name: "LAFIA" },
  { id: 56, abbr: "ISL", name: "LAGOS ISLAND" },
  { id: 2, abbr: "MLD", name: "LAGOS MAINLAND" },
  { id: 33, abbr: "LKJ", name: "LOKOJA" },
  { id: 35, abbr: "MIU", name: "MAIDUGURI" },
  { id: 34, abbr: "MDI", name: "MAKURDI" },
  { id: 36, abbr: "MNA", name: "MINNA" },
  { id: 37, abbr: "NNI", name: "NNEWI" },
  { id: 38, abbr: "NSK", name: "NSUKKA" },
  { id: 39, abbr: "OFA", name: "OFA" },
  { id: 60, abbr: "OGB", name: "OGBOMOSO" },
  { id: 40, abbr: "ONA", name: "ONITSHA" },
  { id: 42, abbr: "OSG", name: "OSHOGBO" },
  { id: 43, abbr: "OTA", name: "OTA" },
  { id: 41, abbr: "ORI", name: "OWERRI" },
  { id: 61, abbr: "OYO", name: "OYO" },
  { id: 44, abbr: "PHC", name: "PORT-HARCOURT" },
  { id: 46, abbr: "SAG", name: "SAGAMU" },
  { id: 45, abbr: "SAE", name: "SAPELE" },
  { id: 47, abbr: "SKO", name: "SOKOTO" },
  { id: 48, abbr: "SUL", name: "SULEJA" },
  { id: 49, abbr: "UHA", name: "UMUAHIA" },
  { id: 50, abbr: "UYO", name: "UYO" },
  { id: 51, abbr: "WRI", name: "WARRI" },
  { id: 52, abbr: "YEN", name: "YENAGOA" },
  { id: 53, abbr: "YOL", name: "YOLA" },
  { id: 54, abbr: "ZRI", name: "ZARIA" },
];
const statesAndCapitals = [
  { state: "Abia", capital: "Umuahia" },
  { state: "Adamawa", capital: "Yola" },
...
const BusinessInfo: FC<general_type> = ({
  ...
  // --- NEW: Sizing & Pricing Assessment State ---
  const [assessmentMetrics, setAssessmentMetrics] = useState({
    employeeCountTier: 0,
    premisesTypeTier: 0,
    monthlyRevenueTier: 0,
    distributionReachTier: 0,
    registrationStatusTier: 0,
  });
  ...
  // --- NEW: Live Classification Calculation for UI Preview ---
  const calculateLiveSizePreview = () => {
    const totalScore = 
      assessmentMetrics.employeeCountTier + 
      assessmentMetrics.premisesTypeTier + 
      assessmentMetrics.monthlyRevenueTier + 
      assessmentMetrics.distributionReachTier + 
      assessmentMetrics.registrationStatusTier;
    
    if (totalScore === 0) return "Pending Evaluation";
    if (totalScore >= 5 && totalScore <= 8) return "Very Small Business";
    if (totalScore >= 9 && totalScore <= 13) return "Small Business";
    if (totalScore >= 14 && totalScore <= 17) return "Medium Business";
    if (totalScore >= 18) return "Large Business";
  ...
  const handleCityChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (cityObj) {
      console.log("Selected city:", cityObj);
      setSelectedCity(cityObj);
      fetchTownFromCity(cityObj.abbr);
    } else {
      setSelectedCity({ id: 0, abbr: "", name: value });
      setTown("");
      setTownId(null);
  ...
  //   ) {
  //     showToast("warning", "Please fill all required basic fields before submitting");
  //     return;
  //   }

  //   // Check new Assessment Metric fields
  //   const isAssessmentComplete = Object.values(assessmentMetrics).every((val) => val > 0);
  //   if (!isAssessmentComplete) {
  //     showToast("warning", "Please complete all Business Classification fields to calculate your fee.");
  //     return;
  //   }
  ...
  //         townId,
  //         businessId: addNewBusiness,
  //         assessmentMetrics // <--- Sending the gathered metrics to Backend API
  //       },
  //       headers: {
  //         Authorization: `Bearer ${localStorage.getItem("userToken")}`,
  //       },
  ...
  //     setTimeout(() => {
  //       showToast("success", res.data.message);
  //       // The backend calculates the fee and updates DB. We move to next section.
  //       setSection(3); 
  //     }, 3000);
  //   } catch (err: any) {
  //     console.error(
...
const handleAPI = async () => {
    if (
      ...
      !selectedCity?.name ||
      !town ||
      !industryCategory.trim() // <-- Added validation for new field
    ) {
      showToast("warning", "Please fill all required basic fields before submitting");
      return;
    }
    ...
    try {
      ...
      formData.append("businessId", addNewBusiness);
      ...
      formData.append("cityId", selectedCity?.id?.toString() || ""); 
      formData.append("street", streetAddress);
      formData.append("zip", zipCode);
      formData.append("town", town);
      if (townId) formData.append("townId", townId.toString());
      
      // Append the new category field
      formData.append("industryCategory", industryCategory);

      // Append arrays and objects as strings
      const selectedRegulations = regulations.filter(({ state }) => state).map(({ item }) => item);
      formData.append("regulations", JSON.stringify(selectedRegulations));
      formData.append("assessmentMetrics", JSON.stringify(assessmentMetrics));
  ...
  useEffect(() => {
    ...
    town,
    stateOfOrigin,
    assessmentMetrics // Re-bind function if metrics change
  ]);
...
// Add to your state declarations
  ...
  useEffect(() => {
    const fetchParentCategories = async () => {
      try {
        // Replace '/categories/tree' with your exact route path if different
        const res = await axios.get("/categories/tree/all", {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("userToken")}`,
          },
        });
        
        // The tree structure naturally keeps parent categories at the root level
        if (res.data && Array.isArray(res.data)) {
          setParentCategories(res.data);
        }
      } catch (err) {
        console.error("Error fetching categories:", err);
        showToast("error", "Failed to load industry categories.");
      }
    };
  ...
  return (
    <div className="py-3">
      <div className="py-5 w-full">
        <div className="py-5 flex flex-col gap-4">
          ...
          <div>
            <select
              ...
              required={true}
              className="w-full px-4 py-3 rounded-xl outline-none bg-white border-[0.67px] border-[#C5C6CC] text-[#1F2024]"
            >
        ...
        {/* --- NEW: BUSINESS ASSESSMENT SECTION --- */}
        <div className="border-[0.5px] border-[#D4D6DD] rounded-2xl p-4 mt-3 mb-6 bg-[#F8F9FA]">
          <h1 className="flex gap-2 items-center pb-2">
            <span className="flex items-center justify-center rounded-full w-4 h-4 bg-[--foreground-green]">
              <span className="flex items-center justify-center rounded-full bg-white h-[5px] w-[5px]"></span>
            </span>
          ...
          <p className="text-xs text-[#8F9098] mb-4">
            These details determine your business verification tier and fees.
          </p>

          <div className="flex flex-col gap-4">
            <select
              name="employeeCountTier"
              value={assessmentMetrics.employeeCountTier}
              onChange={handleMetricChange}
              className="w-full px-4 py-3 rounded-xl outline-none bg-white border-[0.67px] border-[#C5C6CC] text-[#1F2024]"
            >
              <option value={0} disabled>Select Employee Size</option>
              <option value={1}>1 to 5 Employees</option>
              <option value={2}>6 to 20 Employees</option>
              <option value={3}>21 to 50 Employees</option>
              <option value={4}>Above 50 Employees</option>
            </select>

            <select
              name="premisesTypeTier"
              value={assessmentMetrics.premisesTypeTier}
              onChange={handleMetricChange}
              className="w-full px-4 py-3 rounded-xl outline-none bg-white border-[0.67px] border-[#C5C6CC] text-[#1F2024]"
            >
              <option value={0} disabled>Select Premises Type</option>
              <option value={1}>Home Based / Online Only</option>
              <option value={2}>Standard Shop / Kiosk</option>
              <option value={3}>Workshop / Large Store</option>
              <option value={4}>Industrial Factory / Facility</option>
            </select>

            <select
              name="monthlyRevenueTier"
              value={assessmentMetrics.monthlyRevenueTier}
              onChange={handleMetricChange}
              className="w-full px-4 py-3 rounded-xl outline-none bg-white border-[0.67px] border-[#C5C6CC] text-[#1F2024]"
            >
              <option value={0} disabled>Select Estimated Monthly Revenue</option>
              <option value={1}>Under ₦500,000</option>
              <option value={2}>₦500,000 to ₦5,000,000</option>
              <option value={3}>₦5,000,000 to ₦20,000,000</option>
              <option value={4}>Above ₦20,000,000</option>
            </select>

            <select
              name="distributionReachTier"
              value={assessmentMetrics.distributionReachTier}
              onChange={handleMetricChange}
              className="w-full px-4 py-3 rounded-xl outline-none bg-white border-[0.67px] border-[#C5C6CC] text-[#1F2024]"
            >
              <option value={0} disabled>Select Distribution Reach</option>
              <option value={1}>Local / Single City</option>
              <option value={2}>Statewide</option>
              <option value={3}>Multi-State Regional</option>
              <option value={4}>Nationwide / Export</option>
            </select>

            <select
              name="registrationStatusTier"
              value={assessmentMetrics.registrationStatusTier}
              onChange={handleMetricChange}
              className="w-full px-4 py-3 rounded-xl outline-none bg-white border-[0.67px] border-[#C5C6CC] text-[#1F2024]"
            >
              <option value={0} disabled>Select Registration Status</option>
              <option value={1}>Not Registered Yet</option>
              <option value={2}>CAC Registered</option>
              <option value={3}>CAC + Industry Specific Reg.</option>
              <option value={4}>Multiple Regulatory Clearances</option>
            </select>

            {/* Live Visual Preview of the Evaluation */}
            {/* <div className="mt-2 p-3 bg-[#EAF2FF] border border-[#B3D4FF] rounded-xl flex justify-between items-center">
              <span className="text-sm text-[#0047AB] font-semibold">Projected Classification:</span>
              <span className="text-sm text-[#1F2024] font-bold">{calculateLiveSizePreview()}</span>
            </div> */}
          </div>
        </div>

...
const PriceFilter: React.FC<PriceFilterProps> = ({ minPrice, maxPrice, setMinPrice, setMaxPrice, applyFilter }) => {
  ...
  const handleMinChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = Number(e.target.value.replace(/[^0-9]/g, ''));
    setMinPrice(Math.min(value, maxPrice || MAX_RANGE));
  };
  const handleMaxChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = Number(e.target.value.replace(/[^0-9]/g, ''));
    setMaxPrice(Math.max(value, minPrice || MIN_RANGE));
  };
...
/**
 * Hierarchical Category Filter mapping tree updates to URL queries
 */
const CategoryFilter: React.FC<CategoryFilterProps> = ({ selectedCategory, setSelectedCategory }) => {
  ...
  // Sync state with URL parameters on initial execution mount
  const activeIndustry = searchParams.get("industry");
  const activeCategory = searchParams.get("category");
  const activeSub = searchParams.get("sub");
  ...
  const handleFilter = (type: 'industry' | 'category' | 'sub', id: string, name: string) => {
    const params = new URLSearchParams(searchParams.toString());
    
    // Assign locally tracked category string reference 
    setSelectedCategory(name);
    
    // Reset downward levels sequentially when a parent element is triggered
    if (type === 'industry') {
        params.set("industry", id);
        params.delete("category");
        params.delete("sub");
    ...
    <div className="space-y-2 pt-2">
      {tree.map((industry) => (
        <div key={industry._id} className="border-b border-gray-50 pb-2">
          ...
          <div className="flex items-center justify-between group">
            <button
              onClick={() => handleFilter('industry', industry._id, industry.name)}
              className={`text-sm py-2 font-semibold transition-colors ${
                activeIndustry === industry._id ? 'text-orange-600 font-bold' : 'text-gray-800 hover:text-orange-500'
              }`}
            >
              {industry.name.toUpperCase()}
            </button>
          ...
          {expanded.includes(industry._id) && industry.children?.map((cat) => (
            <div key={cat._id} className="ml-4 mt-1">
              <div className="flex items-center justify-between">
                <button
                  onClick={() => handleFilter('category', cat._id, cat.name)}
                  className={`text-xs py-1 transition-colors ${
                    activeCategory === cat._id ? 'text-orange-600 font-bold' : 'text-gray-600 hover:text-orange-500'
                  }`}
                >
                  {cat.name}
                </button>
              ...
              {expanded.includes(cat._id) && cat.children?.map((sub) => (
                <button
                  key={sub._id}
                  onClick={() => handleFilter('sub', sub._id, sub.name)}
                  className={`block ml-4 text-[11px] py-1 transition-colors ${
                    activeSub === sub._id ? 'text-orange-500 font-bold' : 'text-gray-500 hover:text-orange-400'
                  }`}
                >
                  • {sub.name}
                </button>
...
const ProductFilterSidebar: React.FC = () => {
  ...
  // Initialize values from URL processing matrices safely
  const [minPrice, setMinPrice] = useState<number>(
    Number(searchParams.get('minPrice')) || 1000
  );
  const [maxPrice, setMaxPrice] = useState<number>(
  ...
  return (
    <div className="bg-white p-4 w-full max-w-sm rounded-xl shadow-xl space-y-4">
      ...
      {/* 4. Discount Percentage Filter Section */}
      {/* <section>
        <FilterSectionTitle title="DISCOUNT PERCENTAGE" />
        <RadioGroupFilter
          name="Discount Percentage"
      ...
      {/* 5. Product Rating Filter Section */}
      {/* <section>
        <FilterSectionTitle title="PRODUCT RATING" />
        <RatingFilter
          selectedRating={selectedRating}
      ...
      {/* Placeholder advertisement block layout container */}
      <div className="w-full h-32 bg-gray-200 rounded-lg flex items-center justify-center text-gray-500">
        {/* Ads content placeholder layout space */}
      </div>
    </div>
  );
};
